...
def convert_scan_to_xy(scan: LaserScan):
    ...
    theta = np.linspace(scan.angle_min, scan.angle_max, num=len(scan.ranges))
...
def get_twist_values(scan: LaserScan, robot_width = 0.38, stopping_distance = 1.5, max_speed = 0.2):
    ...
    close_scans = np.where(np.abs(ys) <= (robot_width / 2))
    ...
    linear_x = 0.0 if abs(min_dist - stopping_distance) < 0.01 else robot_speed
